...
await c.connect()
// Non-image, NOT-yet-upgraded aptitude-bank rows (post-dedup), with existing solutions.
const { rows } = await c.query(
  `select id, aptitude_type, question_text, question_text_ta, option_a, option_b, option_c, option_d,
     ...
     from questions
    where category='aptitude' and images is null and expl_status is distinct from 'generated'
...
for (const [type, list] of Object.entries(byType)) {
  ...
  for (let i = 0; i * 12 < list.length; i++) {
    ...
    const name = `q_${type}_r_${i}`
    writeFileSync(`${OUT}/${name}.json`, JSON.stringify(slice, null, 1))
    chunks.push({ name, subject: `${type}_r`, count: slice.length })
  }
  writeFileSync(`${OUT}/_chunks_${type}_r.json`, JSON.stringify(chunks, null, 1))
  summary[type] = { questions: list.length, chunks: chunks.length }
}
console.log('remaining non-image to upgrade (post-dedup):', JSON.stringify(summary, null, 1))
await c.end()
