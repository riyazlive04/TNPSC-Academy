-- ============================================================================
-- TNPSC Mentors — per-option figures (option_images) for image-option questions
-- ----------------------------------------------------------------------------
-- A few Group 2 non-verbal questions have FIGURES as their answer options (e.g.
-- "pick the next figure"). `option_images` holds a JSON map of letter → public
-- image URL, e.g. {"A": "...q106_a.png", "B": "..."}. The option button renders
-- the figure when a letter has one, and falls back to its text otherwise (so a
-- mixed question like q154, where D = "None of these", still works).
--
-- Adds the column + grant, and re-creates get_quiz_questions to return it
-- (preserving the year filter from pyq2.sql). Idempotent. Run:
--   node run-migration.mjs ../supabase/pyq2_option_images.sql
-- ============================================================================

alter table public.questions add column if not exists option_images jsonb;
-- Column-level grants gate this project's `questions` reads — new column must be
-- granted or filtering/returning it for `authenticated` is denied.
grant select (option_images) on public.questions to authenticated;

-- RETURNS TABLE gains a column → the function must be dropped before re-create.
drop function if exists public.get_quiz_questions(jsonb);
create function public.get_quiz_questions(p_config jsonb)
returns table (
  ...
  difficulty text, images jsonb, option_images jsonb, source_tag text,
...
as $$
  ...
  select q.id, q.category, q.group_type, q.year, q.standard,
         ...
         q.difficulty, q.images, q.option_images, q.source_tag,
