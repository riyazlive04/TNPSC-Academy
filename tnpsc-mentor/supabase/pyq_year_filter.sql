-- ============================================================================
-- Re-add the exam-YEAR filter to get_quiz_questions / count_quiz_questions
-- ----------------------------------------------------------------------------
-- The live samplers (leak-guard → vettri.sql lineage) dropped the optional `year`
-- filter, so the Group-2 year chip AND the new Group-1 year picker were no-ops.
-- This restores it: `and (cfg.mock or cfg.year is null or q.year = cfg.year)` —
-- a no-op when the config omits `year`, so every existing caller is unchanged.
-- Bodies are otherwise identical to the current live definition (all category
-- guards kept: outer / mock / testseries / vettri). Return types unchanged, so
-- `create or replace` is safe.
--
--   node run-migration.mjs ../supabase/pyq_year_filter.sql
-- ============================================================================
