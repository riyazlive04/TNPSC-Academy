-- ============================================================================
-- Test Series leak guard — recreate ONLY get_quiz_questions + count_quiz_questions
-- ----------------------------------------------------------------------------
-- Adds `and q.category <> 'testseries'` (next to the existing 'mock'/'outer'
-- guards) so the fixed, scheduled Test Series bank never leaks into the general
-- random sampler. These two bodies are copied verbatim from secure.sql with only
-- that one line added, so they can be applied in isolation.
--
-- Apply with:  node run-migration.mjs ../supabase/test_series_leak_guard.sql
-- DO NOT re-run the whole secure.sql (the default `node run-migration.mjs` path
-- currently fails partway and leaves quiz functions dropped — see the
-- mock-exams-gating gotcha).
-- ============================================================================
